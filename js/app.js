...
import {
  ...
  calculateDictionaryMetadata,
  calculateBoxplotAndScatterData
} from './analytics.js';
...
const state = {
  ...
  univariate: {
    ...
    chartType: 'bar' // 'bar' | 'pie' | 'histogram'
  },
  bivariate: {
    varX: 'nivelSupervision',
    varY: 'impactoGlobal',
    vizType: 'Barras Agrupadas' // 'Barras Agrupadas' | 'Barras Apiladas 100%' | 'Mapa de Calor (Heatmap)' | 'Boxplot con Puntos / Dispersión (Jitter)' | 'Radar'
  },
...
const dom = {
  ...
  btnViewHistogram: document.getElementById('btnViewHistogram'),
  ...
  dictSummaryBadges: document.getElementById('dictSummaryBadges'),
  dictionaryContainer: document.getElementById('dictionaryContainer')
};
...
function setupEventListeners() {
  ...
  dom.selectUnivariateVar.addEventListener('change', (e) => {
    ...
    const def = VARIABLE_DEFINITIONS[state.univariate.selectedVar];
    if (def && def.type === 'quantitative') {
      state.univariate.chartType = 'histogram';
    } else if (state.univariate.chartType === 'histogram') {
      state.univariate.chartType = 'bar';
    }
    updateTab2ViewButtons();
    updateTab2();
  });

  // Tab 2: Botones de tipo de gráfico (Histograma vs Barras vs Circular)
  if (dom.btnViewHistogram) {
    dom.btnViewHistogram.addEventListener('click', () => {
      state.univariate.chartType = 'histogram';
      updateTab2ViewButtons();
      updateTab2();
    });
  ...
  // Tab 4: Búsqueda interactiva en el Cuestionario y Diccionario de Datos
  if (dom.dictSearchInput) {
    dom.dictSearchInput.addEventListener('input', (e) => {
      state.dictionarySearchTerm = e.target.value;
      renderTab4Dictionary(state.dictionarySearchTerm);
    });
  }
...
/**
 * Ejecuta el filtrado y actualiza los 4 tabs de forma sincronizada.
 */
function applyFiltersAndRender() {
  ...
  // Actualizar las 4 pestañas de forma sincronizada
  updateTab1();
  updateTab2();
  updateTab3();
  renderTab4Dictionary(state.dictionarySearchTerm || '');
...
function updateTab2() {
  ...
  // 3. Gráfico Univariable (Histograma, Barras o Circular)
  const categories = freqData.rows.map(r => r.category);
  const values = freqData.rows.map(r => r.fi);
  const isQuantitative = def.type === 'quantitative';
  renderUnivariateChart('chartUnivariate', categories, values, state.univariate.chartType, def.label, isQuantitative);
}
...
function updateTab3() {
  ...
  // 2. Calcular datos de Boxplot y Dispersión con Jitter
  const boxplotScatterData = calculateBoxplotAndScatterData(state.filteredData, varX, varY);

  // 3. Renderizar Gráfico Bivariable con los 5 modos disponibles
  renderBivariateChart('chartBivariate', vizType, contingency, boxplotScatterData, defX.label, defY.label);

  // 4. Renderizar Tabla de Contingencia (Matriz de doble entrada)
...
/**
 * Actualiza el estado activo de los botones de visualización en el Tab 2.
 */
function updateTab2ViewButtons() {
  if (dom.btnViewHistogram) {
    dom.btnViewHistogram.classList.toggle('active', state.univariate.chartType === 'histogram');
  }
  if (dom.btnViewBar) {
    dom.btnViewBar.classList.toggle('active', state.univariate.chartType === 'bar');
  }
  if (dom.btnViewPie) {
    dom.btnViewPie.classList.toggle('active', state.univariate.chartType === 'pie');
  }
}

/**
 * Renderiza el Tab 4: Cuestionario y Diccionario de Datos interactivo.
 * @param {string} searchTerm 
 */
function renderTab4Dictionary(searchTerm = '') {
  if (!dom.dictionaryContainer || !dom.dictSummaryBadges) return;

  const metadataList = calculateDictionaryMetadata(state.filteredData);

  // 1. Badges de resumen estadístico de variables
  const totalVars = metadataList.length;
  const quantCount = metadataList.filter(m => m.type === 'quantitative').length;
  const ordinalCount = metadataList.filter(m => m.type === 'ordinal').length;
  const nominalCount = metadataList.filter(m => m.type === 'nominal').length;
  const multiCount = metadataList.filter(m => m.type === 'multi').length;

  dom.dictSummaryBadges.innerHTML = `
    <span class="dict-stat-badge badge-all"><strong>${totalVars}</strong> Variables Totales</span>
    <span class="dict-stat-badge badge-quant"><strong>${quantCount}</strong> Cuantitativa</span>
    <span class="dict-stat-badge badge-ordinal"><strong>${ordinalCount}</strong> Ordinales</span>
    <span class="dict-stat-badge badge-nominal"><strong>${nominalCount}</strong> Nominales</span>
    <span class="dict-stat-badge badge-multi"><strong>${multiCount}</strong> Opción Múltiple</span>
  `;

  // 2. Filtrar variables por el término de búsqueda
  ...
  const filtered = term
    ? metadataList.filter(item => {
        return (
          ...
          (item.exactQuestion && item.exactQuestion.toLowerCase().includes(term)) ||
          (item.typeLabel && item.typeLabel.toLowerCase().includes(term)) ||
          (item.category && item.category.toLowerCase().includes(term)) ||
          (item.scaleDescription && item.scaleDescription.toLowerCase().includes(term))
        );
      })
    : metadataList;

  // 3. Manejo de estado vacío si la búsqueda no arroja resultados
  if (filtered.length === 0) {
    dom.dictionaryContainer.innerHTML = `
      <div class="dict-empty-state">
        <div style="font-size: 32px; margin-bottom: 8px;">🔍</div>
        <h3 style="font-size: 16px; color: var(--text-main); margin-bottom: 6px;">No se encontraron variables o preguntas</h3>
        <p>No hay coincidencias para el término "<strong>${escapeHtml(searchTerm)}</strong>".</p>
        <button class="btn-reset" style="margin-top: 14px; display: inline-flex;" id="btnResetDictSearch">
          ↺ Limpiar búsqueda
        </button>
      </div>
    `;

    const btnResetSearch = document.getElementById('btnResetDictSearch');
    if (btnResetSearch) {
      btnResetSearch.addEventListener('click', () => {
        if (dom.dictSearchInput) dom.dictSearchInput.value = '';
        state.dictionarySearchTerm = '';
        renderTab4Dictionary('');
      });
    }
    return;
  }

  // 4. Renderizar tarjetas de variables
  dom.dictionaryContainer.innerHTML = '';
  filtered.forEach(item => {
    let pillClass = 'pill-category';
    if (item.type === 'quantitative') pillClass = 'pill-quant';
    else if (item.type === 'ordinal') pillClass = 'pill-ordinal';
    else if (item.type === 'nominal') pillClass = 'pill-nominal';
    else if (item.type === 'multi') pillClass = 'pill-multi';

    // Generar tags para las categorías observadas en los datos filtrados
    let activeTagsHtml = '';
    if (item.cardinality === 0) {
      activeTagsHtml = `<span style="font-size: 11px; color: var(--text-muted);">Sin valores registrados con los filtros actuales</span>`;
    } else {
      const tags = (item.sampleValues || []).map(val =>
        `<span class="dict-tag" title="${escapeHtml(val)}">${escapeHtml(val)}</span>`
      ).join('');
      const more = item.cardinality > (item.sampleValues || []).length
        ? `<span class="dict-tag" style="color: var(--primary); font-weight: 600;">+${item.cardinality - item.sampleValues.length} más</span>`
        : '';
      activeTagsHtml = `<div class="dict-active-tags">${tags}${more}</div>`;
    }
    ...
    card.innerHTML = `
      <div class="dict-card-header">
        <div class="dict-card-header-left">
          <span class="badge-code">${item.code || 'P'}</span>
          <span class="badge-var-name">${item.key}</span>
          <span class="badge-pill ${pillClass}">${item.typeLabel || item.type}</span>
          <span class="badge-pill pill-category">${item.category}</span>
        </div>
        <div class="dict-card-header-right">
          <span class="badge-cardinality" title="Categorías distintas detectadas en la muestra activa">
            <span>Cardinalidad activa:</span>
            <strong>${item.cardinality}</strong>
          </span>
        </div>
      </div>

      <div class="dict-question-text">
        "${escapeHtml(item.exactQuestion)}"
      </div>

      <div class="dict-card-body">
        <div class="dict-meta-block">
          <div class="dict-meta-label">
            <span>📏</span> Escala / Opciones de Medición
          </div>
          <div class="dict-meta-content">${escapeHtml(item.scaleDescription)}</div>
        </div>

        <div class="dict-meta-block">
          <div class="dict-meta-label">
            <span>🏷️</span> Categorías Activas en Muestra Filtrada (${item.cardinality})
          </div>
          ${activeTagsHtml}
        </div>
      </div>
